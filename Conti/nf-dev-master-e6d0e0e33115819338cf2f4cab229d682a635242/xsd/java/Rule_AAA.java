...
public class Rule_AAA extends NetworkObject {
  ...
  @Override protected void init(  Context ctx,  Object[]... args){
    ...
    matchEntry=ctx.mkFuncDecl(n_AAA + "_matchEntry",new Sort[]{ctx.mkIntSort(),ctx.mkIntSort()},ctx.mkBoolSort());
  ...
  @Override protected void addConstraints(  Solver solver){
    ...
    Expr e_1=ctx.mkIntConst(n_AAA + "_entry_e_1");
  ...
  public void installAAA(  Expr AUTHENTICATION_PORT_1812,  Expr ACCESS_REJECT,  Expr AUTHORIZATION_RESPONSE,  Expr ACCOUNTING_PORT_1813,  Expr ACCOUNTING_RESPONSE){
    ...
    constraints.add(ctx.mkForall(new Expr[]{t_0,p_0,n_0},ctx.mkImplies((BoolExpr)nctx.send.apply(n_AAA,n_0,p_0,t_0),ctx.mkExists(new Expr[]{t_1,p_1,n_1},ctx.mkAnd((BoolExpr)nctx.recv.apply(n_1,n_AAA,p_1,t_1),ctx.mkLt(t_1,t_0),ctx.mkNot(ctx.mkEq(nctx.pf.get("dst_port").apply(p_1),AUTHENTICATION_PORT_1812)),ctx.mkEq(nctx.pf.get("dst_port").apply(p_1),ACCOUNTING_PORT_1813),ctx.mkEq(nctx.pf.get("src").apply(p_0),nctx.pf.get("dest").apply(p_1)),ctx.mkEq(nctx.pf.get("src_port").apply(p_0),nctx.pf.get("dst_port").apply(p_1)),ctx.mkEq(nctx.pf.get("dest").apply(p_0),nctx.pf.get("src").apply(p_1)),ctx.mkEq(nctx.pf.get("dst_port").apply(p_0),nctx.pf.get("src_port").apply(p_1)),ctx.mkEq(nctx.pf.get("application_data").apply(p_0),ACCOUNTING_RESPONSE),ctx.mkEq(nctx.pf.get("transport_protocol").apply(p_0),nctx.pf.get("transport_protocol").apply(p_1)),ctx.mkEq(nctx.pf.get("proto").apply(p_0),nctx.pf.get("proto").apply(p_1))),1,null,null,null,null)),1,null,null,null,null));
  }
}
