...
public class IfConditionVisitor extends ASTVisitor{
	...
	public IfConditionVisitor(NFdefinition nfd) {
		this.nfd = nfd;
	}
	...
	public boolean visit(InfixExpression node) {
		this.analize=false; //I use it to not continue the recursive analysis of the same expression with the other two methods
		ArrayList<Expression> ie = new ArrayList<Expression>();
		ie.add(node.getLeftOperand());
		ie.add(node.getRightOperand());
		ie.addAll(node.extendedOperands());
		for(Expression e : ie) {
			ConditionVisitor cv = new ConditionVisitor(this.nfd);
			e.accept(cv); //inspect the condition
			if(cv.isNot()==true) {
				cv.getCondition().add("not"); //insert not as the last element of the condition
			}
			ifConditions.add(cv.getCondition());
		}
		return super.visit(node);
	}
	
	@Override
	public boolean visit(MethodInvocation node) { //if there is only one expression
		if(this.analize) {
			this.analize=false;
			ConditionVisitor cv = new ConditionVisitor(this.nfd);
			node.accept(cv);
			if(cv.isNot()==true) {
				cv.getCondition().add("not");
			}
			ifConditions.add(cv.getCondition());
		}
		return super.visit(node);
	...
	@Override
	public boolean visit(PrefixExpression node) { //if there is only one expression
		if(this.analize) {
			this.analize=false;
			ConditionVisitor cv = new ConditionVisitor(this.nfd);
			node.accept(cv);
