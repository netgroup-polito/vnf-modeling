...
public class NewNetFunc2 extends EndHost{
	...
	public Packet defineSendingPacket() {
		...
		packet.setOrigin((Address) (this.hostPropertyList.get("ip_mailServer")));
		return packet;
	}

	@Override
	public void defineState() {
		this.state.hostTableList.add(HostTable.createTable("Blacklist",2, FieldType.ORIGIN));
	...
	public RoutingResult onReceivedPacket(Packet packet) {
		if(Packet.match(packet.getProtocol(), Constants.HTTP_RESPONSE_PROTOCOL, Operator.EQUAL) &&
			...
			if(Packet.match(packet.getOrigin(), (Address)(this.hostPropertyList.get("ip_mailServer")), Operator.EQUAL)
					) {
