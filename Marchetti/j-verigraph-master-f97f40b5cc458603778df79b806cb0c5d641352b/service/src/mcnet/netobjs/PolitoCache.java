...
public class PolitoCache extends NetworkObject{
	    ...
	    public void installCache (NetworkObject[] internalNodes){
	    	Expr n_0 = ctx.mkConst("politoCache_"+node+"_n_0", nctx.node);
	    	Expr n_1 = ctx.mkConst("politoCache_"+node+"_n_1", nctx.node);
	    	Expr n_2 = ctx.mkConst("politoCache_"+node+"_n_2", nctx.node);
	    	
	    	Expr p_0 = ctx.mkConst("politoCache_"+node+"_p_0", nctx.packet);
	    	Expr p_1 = ctx.mkConst("politoCache_"+node+"_p_1", nctx.packet);
	    	Expr p_2 = ctx.mkConst("politoCache_"+node+"_p_2", nctx.packet);
	    	
	    	IntExpr t_0 = ctx.mkIntConst("politoCache_"+node+"_t_0");
	    	IntExpr t_1 = ctx.mkIntConst("politoCache_"+node+"_t_1");
	    	IntExpr t_2 = ctx.mkIntConst("politoCache_"+node+"_t_2");
	    	            
	    	Expr a_0 = ctx.mkConst(node+"politoCache_a_0", nctx.node);
	    	IntExpr u_0 = ctx.mkIntConst("politoCache_"+node+"_u_0");
	    	
	    	FuncDecl isInternalNode = ctx.mkFuncDecl(node+"_isInternalNode", nctx.node, ctx.mkBoolSort());
	    	FuncDecl isInCache = ctx.mkFuncDecl(node+"_isInCache", new Sort[]{ctx.mkIntSort(),ctx.mkIntSort()}, ctx.mkBoolSort());
...
//	                                ctx.mkLt(t_1, t_0),
//	                                ctx.mkLt(t_2, t_0),
//	                                (BoolExpr)nctx.recv.apply(n_1, node, p_1, t_1),
//	                                (BoolExpr)nctx.recv.apply(n_2, node, p_2, t_2),
...
//	    	 
	    	...
	    	//						    	(   t_1< t_2 < t_0 && recv(n_1, node, p_1, t_1) && recv(n_2, node, p_2, t_2))) && 
...
//	    	constraints.add(ctx.mkForall(new Expr[]{u_0, t_0},ctx.mkEq(isInCache.apply(u_0,t_0),ctx.mkBool(true)),1,null,null,null,null));
	    	...
	    	//				send(node, n_0, p, t_0) && !isInternalNode(n_0) -> 
			//	    			(exist t_1,n_1 :
			//	    		    (t_1 < t_0 && recv(n_1, node, p, t_1) &&  
			//	    		     p.proto == HTTP_REQ &&  !isInCache(p.url,t_0)) 
	    	constraints.add(
	    			ctx.mkForall(new Expr[]{n_0,p_0, t_0},
	   	    			 ctx.mkImplies(
	   	    					 ctx.mkAnd((BoolExpr)nctx.send.apply(node,n_0,p_0,t_0),ctx.mkNot((BoolExpr)isInternalNode.apply(n_0))),
	   	    					 ctx.mkAnd(ctx.mkExists(new Expr[]{t_1, n_1}, 
	   	    							 ctx.mkAnd(
	   	    									 ctx.mkLt(t_1, t_0),
	   	    									 (BoolExpr)isInternalNode.apply(n_1),
	   	    									 (BoolExpr)nctx.recv.apply(n_1, node, p_0, t_1)),1,null,null,null,null),
	    	...
	    	//Constraint4		send(node, n_0, p, t_0) && isInternalNode(n_0) -> 
			//	    			(exist p_1,t_1 : 
			//	    		    	(t_1 < t_0 && recv(n_0, node, p_1, t_1) &&  
	    	...
	    	constraints.add(
	    			ctx.mkForall(new Expr[]{n_0,p_0, t_0},
	   	    			 ctx.mkImplies(
	   	    					 ctx.mkAnd((BoolExpr)nctx.send.apply(node,n_0,p_0,t_0),(BoolExpr)isInternalNode.apply(n_0)),
...
public class PolitoCache extends NetworkObject{
		...
		public void setInternalAddress(ArrayList<NetworkObject> internalAddress){
			...
			assert(internalAddress.size()!=0);
//			constraints.add(
//	   			 ctx.mkForall(new Expr[]{n_0}, 
//	   					 ctx.mkIff((BoolExpr)private_addr_func.apply(n_0), ctx.mkOr(constr.toArray(constrs))),1,null,null,null,null));
			constraints.add(ctx.mkForall(new Expr[]{n_0}, ctx.mkEq(isInternalNode.apply(n_0),ctx.mkOr(constr.toArray(constrs))),1,null,null,null,null));
		}
	    ...
	    public void installCache (){
	    	...
	    	isInternalNode = ctx.mkFuncDecl(node+"_isInternalNode", nctx.node, ctx.mkBoolSort());
	    	FuncDecl isInCache = ctx.mkFuncDecl(node+"_isInCache", new Sort[]{ctx.mkIntSort(),ctx.mkIntSort()}, ctx.mkBoolSort());
...
//	    	constraints.add(ctx.mkForall(new Expr[]{u_0, t_0},ctx.mkImplies(
//	    			(BoolExpr)isInCache.apply(u_0, t_0), 
//	    					 ctx.mkExists(new Expr[]{t_1,t_2,p_1,p_2,n_1, n_2},ctx.mkAnd(
//	    									 ctx.mkLt(t_1, t_2), //t1<t2<t0
//	    									 ctx.mkLt(t_1, t_0),
//	    									 ctx.mkLt(t_2, t_0),
//	    									 (BoolExpr)nctx.recv.apply(n_1, node, p_1, t_1),//
//	    									 (BoolExpr)nctx.recv.apply(n_2, node, p_2, t_2),//
...
//	    									 ctx.mkNot((BoolExpr)isInternalNode.apply(n_2)),//
//	    									 ctx.mkEq(nctx.pf.get("url").apply(p_1), u_0),//
//	    									 ctx.mkEq(nctx.pf.get("url").apply(p_2), u_0))//
//	    							 ,1,null,null,null,null)),1,null,null,null,null));
	    	
			constraints.add(ctx.mkForall(new Expr[]{t_2,u_0},ctx.mkImplies(
			(BoolExpr)isInCache.apply(u_0, t_2),
			ctx.mkExists(new Expr[]{t_1,t_0,n_1,p_1,p_2,n_2},ctx.mkAnd(
					 ctx.mkLt(t_1, t_2), //t1<t2<t0
					 ctx.mkLt(t_0, t_1),
					 ctx.mkLt(t_0, t_2),
					(BoolExpr)nctx.recv.apply(n_1, node, p_1, t_0),//
					(BoolExpr)nctx.recv.apply(n_2, node, p_2, t_1),//
					ctx.mkEq(nctx.pf.get("proto").apply(p_1), ctx.mkInt(nctx.HTTP_REQUEST)),//
					ctx.mkEq(nctx.pf.get("proto").apply(p_2), ctx.mkInt(nctx.HTTP_RESPONSE)),//
					((BoolExpr)isInternalNode.apply(n_1)),//
					ctx.mkNot(((BoolExpr)isInternalNode.apply(n_2))),//
					ctx.mkEq(nctx.pf.get("url").apply(p_1), u_0),//
//					ctx.mkEq(nctx.pf.get("url").apply(p_2), u_0)//
					ctx.mkEq(nctx.pf.get("url").apply(p_2), nctx.pf.get("url").apply(p_1))
					),1,null,null,null,null)),1,null,null,null,null));
...
//	    			ctx.mkNot((BoolExpr)isInternalNode.apply(n_0))),
//	   	    					 ctx.mkExists(new Expr[]{t_1, n_1},ctx.mkAnd(
//	   	    									 ctx.mkLt(t_1, t_0),
//	   	    									 (BoolExpr)isInternalNode.apply(n_1),
//	   	    									 (BoolExpr)nctx.recv.apply(n_1, node, p_0, t_1),
//	   	    									 ctx.mkEq(nctx.pf.get("proto").apply(p_0), ctx.mkInt(nctx.HTTP_REQUEST)),
//	   	    									 ctx.mkNot((BoolExpr)isInCache.apply(nctx.pf.get("url").apply(p_0), t_0))
//	   	    									 ),1,null,null,null,null)),1,null,null,null,null));
	    	
	    	constraints.add(ctx.mkForall(new Expr[]{t_2,p_0,n_0},ctx.mkImplies(ctx.mkAnd(
					(BoolExpr)nctx.send.apply(node, n_0, p_0, t_2),
					ctx.mkNot(((BoolExpr)isInternalNode.apply(n_0)))),
					ctx.mkExists(new Expr[]{t_1,n_1},ctx.mkAnd(
							ctx.mkLt(t_1, t_2),
							(BoolExpr)isInternalNode.apply(n_1),
							(BoolExpr)nctx.recv.apply(n_1, node, p_0, t_1),
							ctx.mkEq(nctx.pf.get("proto").apply(p_0), ctx.mkInt(nctx.HTTP_REQUEST)),
							ctx.mkNot((BoolExpr)isInCache.apply(nctx.pf.get("url").apply(p_0),t_1))
							),1,null,null,null,null)),1,null,null,null,null));

	    	//Constraint4		send(node, n_0, p_0, t_0) && isInternalNode(n_0) -> 
			//	    			(exist p_1,t_1 : 
			//	    		    	(t_1 < t_0 && recv(n_0, node, p_1, t_1) &&  
			//	    		     	p_1.proto == HTTP_REQ && p_0.proto == HTTP_RESP && 
			//	    		     	p_1.url == p_0.url &&  p_0.src == p_1.dest && p_0.dest==p_1.src 
			//	    		     	&& isInCache(p.url,t_0)) 
	    	
//	    	constraints.add(
//	    			ctx.mkForall(new Expr[]{n_0,p_0, t_0},
//	   	    			 ctx.mkImplies(
//	   	    					 ctx.mkAnd((BoolExpr)nctx.send.apply(node,n_0,p_0,t_0),
//	   	    					(BoolExpr)isInternalNode.apply(n_0)),
...
//	   	    							 ctx.mkEq(nctx.pf.get("src").apply(p_0), nctx.pf.get("dest").apply(p_1)),
//	   	    							 ctx.mkEq(nctx.pf.get("dest").apply(p_0), nctx.pf.get("src").apply(p_1)),
//		    							(BoolExpr)isInCache.apply(nctx.pf.get("url").apply(p_0), t_0)),1,null,null,null,null)),1,null,null,null,null));
	    	
	    	constraints.add(ctx.mkForall(new Expr[]{t_2,p_0,n_0},ctx.mkImplies(ctx.mkAnd(
					(BoolExpr)nctx.send.apply(node, n_0, p_0, t_2),
					((BoolExpr)isInternalNode.apply(n_0))),
					ctx.mkExists(new Expr[]{t_0,t_1,p_1},ctx.mkAnd(
							ctx.mkLt(t_1, t_2),
							ctx.mkLt(t_0, t_1),
							ctx.mkLt(t_0, t_2),
							(BoolExpr)nctx.recv.apply(n_0, node, p_1, t_1),
							ctx.mkEq(nctx.pf.get("proto").apply(p_1), ctx.mkInt(nctx.HTTP_REQUEST)),
							(BoolExpr)isInCache.apply(nctx.pf.get("url").apply(p_1),t_0),
							ctx.mkEq(nctx.pf.get("src").apply(p_0),nctx.pf.get("dest").apply(p_1)),
							ctx.mkEq(nctx.pf.get("dest").apply(p_0),nctx.pf.get("src").apply(p_1)),
							ctx.mkEq(nctx.pf.get("url").apply(p_0),nctx.pf.get("url").apply(p_1)),
							ctx.mkEq(nctx.pf.get("proto").apply(p_0), ctx.mkInt(nctx.HTTP_RESPONSE))),1,null,null,null,null)),1,null,null,null,null));
	    	
			constraints.add(ctx.mkForall(new Expr[]{n_0, p_0, t_2},ctx.mkImplies(
					(BoolExpr)nctx.send.apply(node, n_0, p_0, t_2),ctx.mkOr(
							ctx.mkEq(nctx.pf.get("proto").apply(p_0), ctx.mkInt(nctx.HTTP_REQUEST))
							,ctx.mkEq(nctx.pf.get("proto").apply(p_0), ctx.mkInt(nctx.HTTP_RESPONSE))
