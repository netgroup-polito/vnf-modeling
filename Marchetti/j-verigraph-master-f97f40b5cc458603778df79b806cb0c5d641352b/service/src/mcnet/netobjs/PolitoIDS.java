...
public class PolitoIDS extends NetworkObject {
	...
	public void installIDS(int[] blackList){
		...
		Expr b_0 = ctx.mkIntConst(node + "_b_0");
		...
		//Constraint2    		send(node, n_0, p, t_0) && p.proto(HTTP_RESPONSE) -> 
        //						(exist  n_1,t_1 : (recv(n_1, node, p, t_1) && t_1 < t_0)) && !isInBlackList(p.body)
		
		this.constraints.add(ctx.mkForall(new Expr[]{n_0, p_0, t_0}, 
										  ctx.mkImplies(ctx.mkAnd((BoolExpr)nctx.send.apply(node, n_0, p_0, t_0), ctx.mkEq(nctx.pf.get("proto").apply(p_0), ctx.mkInt(nctx.HTTP_RESPONSE))), 
												  		ctx.mkAnd(ctx.mkExists(new Expr[]{n_1,t_1}, 
												  							   ctx.mkAnd((BoolExpr)nctx.recv.apply(n_1,node,p_0,t_1),ctx.mkLt(t_1, t_0)),
												  							   1,
												  							   null, null, null, null),
		...
		//Constraint3		send(node, n_0, p, t_0) && p.proto(HTTP_REQUEST) -> 
        //						(exist n_1,t_1 : (recv(n_1, node, p, t_1) && t_1 < t_0))
		
		this.constraints.add(ctx.mkForall(new Expr[]{n_0, p_0, t_0}, 
										  ctx.mkImplies(ctx.mkAnd((BoolExpr)nctx.send.apply(node, n_0, p_0, t_0), ctx.mkEq(nctx.pf.get("proto").apply(p_0), ctx.mkInt(nctx.HTTP_REQUEST))), 
												  		ctx.mkAnd(ctx.mkExists(new Expr[]{n_1,t_1}, 
												  							   ctx.mkAnd((BoolExpr)nctx.recv.apply(n_1,node,p_0,t_1),ctx.mkLt(t_1, t_0)),
												  							   1,
												  							   null, null, null, null))),
		...
		//Constraint5		send(node, n_0, p, t_0) -> p.proto == HTTP_REQ || p.protpo == HTTP_RESP
        ...
        this.constraints.add(ctx.mkForall(new Expr[]{n_0, p_0, t_0}, 
                						  ctx.mkImplies((BoolExpr)nctx.send.apply(node, n_0, p_0, t_0), 
                								  		ctx.mkNot((BoolExpr)nctx.nodeHasAddr.apply(node,nctx.pf.get("src").apply(p_0)))),
                						  1,
                						  null,null,null,null));
