...
public class NewNetFunc2 extends NetworkObject{
	...
	private void Blacklist2_Constraints(Solver solver){
		if (Blacklist2_list.size() == 0)
			return;
		IntExpr a_0 = ctx.mkIntConst("NewNetFunc2_"+node+"_a_0");
	...
	private void Blacklist_Constraints(Solver solver){
		if (Blacklist_list.size() == 0)
			return;
		Expr a_0 = ctx.mkConst("NewNetFunc2_"+node+"_a_0",nctx.node);
	...
	public void NewNetFunc2_install(Object ip_mailServer){
		...
		IntExpr t_0 = ctx.mkIntConst(node+"_NewNetFunc2_t_0");
		IntExpr t_1 = ctx.mkIntConst(node+"_NewNetFunc2_t_1");
		IntExpr t_2 = ctx.mkIntConst(node+"_NewNetFunc2_t_2");
		Expr packet = ctx.mkConst(node+"_NewNetFunc2_packet",nctx.packet);
		Expr n_source = ctx.mkConst(node+"_NewNetFunc2_n_source",nctx.node);
		Expr n_0 = ctx.mkConst(node+"_NewNetFunc2_n_0",nctx.node);
		constraints.add(ctx.mkForall(new Expr[]{t_2,packet,n_0},ctx.mkImplies(ctx.mkAnd(
				...
				ctx.mkEq(nctx.pf.get("origin").apply(packet), ((NetworkObject)ip_mailServer).getZ3Node())),
		...
		constraints.add(ctx.mkForall(new Expr[]{t_2,packet,n_0},ctx.mkImplies(ctx.mkAnd(
				...
				ctx.mkExists(new Expr[]{t_0,t_1,packet,n_source},ctx.mkAnd(
						...
						ctx.mkEq(nctx.pf.get("proto").apply(packet), ctx.mkInt(nctx.HTTP_RESPONSE)),
						ctx.mkNot((BoolExpr)Blacklist_func.apply(nctx.pf.get("origin").apply(packet))),
						(BoolExpr)Blacklist2_func.apply((IntExpr)nctx.src_port.apply(packet))),1,null,null,null,null)),1,null,null,null,null));
