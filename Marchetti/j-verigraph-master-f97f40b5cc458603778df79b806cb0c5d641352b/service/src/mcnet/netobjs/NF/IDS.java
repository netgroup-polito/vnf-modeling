...
public class IDS extends NetworkObject{
FuncDecl Blacklist_func;
ArrayList<Integer> Blacklist_list;
public IDS(Context ctx, Object[]... args) {
super(ctx, args);
}
@Override
...
return;
IntExpr a_0 = ctx.mkIntConst("IDS_"+node+"_a_0");
...
public void IDS_install(){
Blacklist_func = ctx.mkFuncDecl(node+"_Blacklist_func",ctx.mkIntSort(), ctx.mkBoolSort());  
IntExpr t_0 = ctx.mkIntConst(node+"_IDS_t_0");
IntExpr t_1 = ctx.mkIntConst(node+"_IDS_t_1");
IntExpr t_2 = ctx.mkIntConst(node+"_IDS_t_2");
Expr packet = ctx.mkConst(node+"_IDS_packet",nctx.packet);
Expr n_source = ctx.mkConst(node+"_IDS_n_source",nctx.node);
Expr n_0 = ctx.mkConst(node+"_IDS_n_0",nctx.node);
...
ctx.mkEq(nctx.pf.get("proto").apply(packet), ctx.mkInt(nctx.HTTP_RESPONSE)),
ctx.mkNot((BoolExpr)Blacklist_func.apply(nctx.pf.get("body").apply(packet)))),1,null,null,null,null)),1,null,null,null,null));
constraints.add(ctx.mkForall(new Expr[]{t_2,packet,n_0},ctx.mkImplies(ctx.mkAnd(
(BoolExpr)nctx.send.apply(node, n_0, packet, t_2),
