...
public class MailClient extends NetworkObject{
public MailClient(Context ctx, Object[]... args) {
super(ctx, args);
}
@Override
...
public void MailClient_install(Object ip_mailServer){
IntExpr t_0 = ctx.mkIntConst(node+"_MailClient_t_0");
IntExpr t_1 = ctx.mkIntConst(node+"_MailClient_t_1");
IntExpr t_2 = ctx.mkIntConst(node+"_MailClient_t_2");
Expr packet = ctx.mkConst(node+"_MailClient_packet",nctx.packet);
Expr n_0 = ctx.mkConst(node+"_MailClient_n_0",nctx.node);
...
ctx.mkEq(nctx.pf.get("dest").apply(packet), ((DatatypeExpr)ip_mailServer))),1,null,null,null,null)),1,null,null,null,null));
