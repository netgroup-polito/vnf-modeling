...
public class Rule_Router extends NetworkObject {
  ...
  @Override protected void addConstraints(  Solver solver){
    ...
    solver.add(ctx.mkForall(new Expr[]{e_0,e_1},ctx.mkEq(matchEntry.apply(e_0,e_1),
    		ctx.mkOr(entry_map)),1,null,null,null,null));
  }
  ...
  public void installRouter(){
    ...
    constraints.add(ctx.mkForall(new Expr[]{t_0,p_0,n_0},
    		ctx.mkImplies((BoolExpr)nctx.send.apply(n_Router,n_0,p_0,t_0),
    ctx.mkExists(new Expr[]{t_1,p_1,n_1},ctx.mkAnd((BoolExpr)nctx.recv.apply(n_1,n_Router,p_1,t_1),ctx.mkLt(t_1,t_0),
    ...
    ctx.mkEq(nctx.pf.get("transport_protocol").apply(p_0),nctx.pf.get("transport_protocol").apply(p_1)),
    ctx.mkEq(nctx.pf.get("proto").apply(p_0),nctx.pf.get("proto").apply(p_1)),
    ctx.mkEq(nctx.pf.get("application_data").apply(p_0),nctx.pf.get("application_data").apply(p_1))),1,null,null,null,null)),1,null,null,null,null));
  }
}
